...
$(document).ready(function($){
	...
	// var $mapWrapper = $('#map'), draggableOp;  //  google maps
    //
    //
	// if ( jQuery.browser.mobile === true ) {
	// 	draggableOp = false;
	...
	// 		infoWindow: {
	// 			content: '<p>Mafiband :), Minsk, Belarus</p>'
	// 		}
	// 	});
	// }
...
$(window).load(function(){
	...
	if ( $('#protfolio-msnry').length > 0 ) {
		...
		var portfolioModal = $('#portfolioModal'),
			...
			portLinkSecond = portfolioModal.find('.modal-footer .modal-action-second'),
			closeLink = portfolioModal.find('.modal-footer .modal-close');
		...
		$('#protfolio-msnry').delegate('a.modal-trigger', 'click', function(e){
			...
			portfolioModal.openModal({
				...
				ready: function() {
					...
					demoLink2 = $this.data('demo-link2');
					...
					if(!demoLink2 || 0 === demoLink2.length) {	// second link empty
						portLink.text("Life Demo");
						portLinkSecond.attr('href', "#");
						portLinkSecond.hide();
					} else {
						portLink.text("Android Live Demo");
						portLinkSecond.text("iOS Live Demo");
						portLinkSecond.attr('href', demoLink2);
						portLinkSecond.show();
					}
