...
// author.myStory comes straight from author.json.file (already resolved by
// index.js, {{years}} replaced with author.experienceYears) - each entry is
// a paragraph, and pdfmake renders embedded "\n" as line breaks within it.
function buildMyStory(myStory) {
    return [sectionTitle('My Story')].concat(
        (myStory || []).map(function (paragraph) {
            return { text: paragraph, style: 'body', margin: [0, 0, 0, 8] };
        })
    );
}
...
// Filesystem/HTTP-header-safe timestamp (no spaces or colons) for use in
// the downloaded file name - see index.js.
function formatFilenameTimestamp(date) {
    return date.getUTCFullYear() + '-' + pad(date.getUTCMonth() + 1) + '-' + pad(date.getUTCDate()) +
        '_' + pad(date.getUTCHours()) + pad(date.getUTCMinutes()) + 'UTC';
}
...
function getDocDefinition(data) {
    ...
    return {
        info: {
            ...
            author: author.name,
            subject: 'Generated on ' + formatGeneratedAt(generatedAt)
        },
        pageSize: 'A4',
...
module.exports = {
    ...
    formatFilenameTimestamp: formatFilenameTimestamp
};
