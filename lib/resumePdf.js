...
// frozen, and this text is editorial copy, not resume data. The years-of-
// experience figure is the one dynamic bit - it comes from
// author.experienceYears (computed in index.js the same way as author.age).
function buildMyStoryParagraphs(experienceYears) {
    return [
    "I'm a Mobile and Web Application developer. My experience is " + experienceYears + " years of creating web-based applications and applications for mobile devices.",
...
function buildMyStory(experienceYears) {
    return [sectionTitle('My Story')].concat(
        buildMyStoryParagraphs(experienceYears).map(function (line) {
...
function getDocDefinition(data) {
    ...
    return {
        ...
        content: [].concat(
            buildHeader(author),
            buildMyStory(author.experienceYears),
