...
function enrichAuthor(author, now) {
    ...
    // schema.org Person JSON-LD for SEO (emitted in the page head). Contains
    // NO phone/email on purpose - same anti-scraping stance as the rendered
    // markup; only already-public info (name, site, socials, city).
    author.structuredData = buildStructuredData(author);

    return author;
}

function buildStructuredData(author) {
    var addressParts = String(author.address || '').split(',').map(function (part) {
        return part.trim();
    });
    ...
    var person = {
        ...
        url: author.site,
        image: author.site + '/' + author.avatar,
        sameAs: Object.keys(author.social || {}).map(function (key) {
            return author.social[key];
        })
    };
